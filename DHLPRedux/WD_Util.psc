...
Armor Property beltRusted Auto
Armor Property beltRustedRendered Auto
...
bool Function AddRapist(Actor akActor)
	...
	If !keyCheckDone && config.requireKeys
		customKey = GetCustomWornBeltKey() ; returns none if the key is the generic or rusty one
		keyCheckDone = true
		checkingKey = false
	EndIf
	if akActor.GetItemCount(libs.chastityKey) > 0
		...
		rapistHasKey = true
	endIf
	;if akActor.GetItemCount(rustyKey) > 0 ; like this'll ever happen
	;	log(akActor.GetLeveledActorBase().GetName() + " has a rusty key!")
	;	rapistHasRustykey = true
	;endIf
...
Function RemoveKeys()
;	If config.stealKeys && ( pl.GetItemCount(libs.chastityKey) > 0 || pl.GetItemCount(libs.restraintsKey) > 0 || pl.GetItemCount(rustyKey) > 0 || ( customKey && pl.GetItemCount(customKey) > 0 ) )
	If config.stealKeys && ( pl.GetItemCount(libs.chastityKey) > 0 || pl.GetItemCount(libs.restraintsKey) > 0 || ( customKey && pl.GetItemCount(customKey) > 0 ) )
		...
		EndIf
;		If pl.GetItemCount(rustyKey) > 0
;			if pl.GetItemCount(beltRustedRendered) > 0
;				; If the player is also wearing the rusted belt, destroy one key
;				; "breaking it on use"
;				pl.RemoveItem(rustyKey, 1, true)
;			endIf
;			log("Player has rusty key")
;			rapistHasRustykey = true
;		EndIf
		...
		;pl.RemoveItem(rustyKey, pl.GetItemCount(rustyKey), true, thief)
		If customKey
...
;	log("GetCustomWornBeltKey()")
	; Redo this at some point if GetDeviceKey() gets performance improvements
	; returns none if the key is the generic from DDi or the rusty one
	If !pl.WornHasKeyword(libs.zad_DeviousBelt)
		return none
	EndIf
	int i = 15 
