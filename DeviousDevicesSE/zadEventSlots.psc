...
Function UpdateGlobalEvent()
	...
	if !libs.GlobalEventFlag
		...
		DoRegister()
	elseif (StorageUtil.GetFormValue(libs.playerref, "DDC_DeviceUsed") As ObjectReference) != None
		libs.Log("Event processing paused due to player locked in furniture device.")
		DoRegister()	
	else
		if ProcessNum == 0
			CheckAllEvents()
